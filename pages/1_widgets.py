...
with st.sidebar:
    lang = st.radio("🌐", ["🇫🇷 FR", "🇬🇧 EN"], horizontal=True, key="lang_w", label_visibility="collapsed")
    FR = "EN" not in lang
    st.divider()
...
if FR:
    ...
    st.caption("Les widgets sont les briques de base de toute interaction dans Streamlit.")
    st.markdown("Chaque widget retourne une **valeur Python** utilisable directement — pas de callbacks, juste une variable.")
else:
    st.title("🎛️ Widgets")
    st.caption("Widgets are the building blocks of every interaction in Streamlit.")
...
with col1:
    if st.button("Clique ici" if FR else "Click here", type="primary"):
        st.success("✅ Cliqué !" if FR else "✅ Clicked!")
...
with col1:
    choix = st.selectbox(
        "Ta librairie préférée ?" if FR else "Favourite library?",
...
with col1:
    ...
    if nom.strip():
        st.write(f"Bienvenue, **{nom.strip()}** 👋" if FR else f"Welcome, **{nom.strip()}** 👋")
with col2:
...
with col1:
    ...
    if show:
        st.markdown("> 📋 " + ("Détails visibles." if FR else "Details visible."))
    else:
        st.markdown("> *(" + ("masqué" if FR else "hidden") + ")* ")
with col2:
...
with col1:
    stack = st.multiselect(
        "Ton stack ?" if FR else "Your stack?",
